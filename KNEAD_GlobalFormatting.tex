%%% SVN stuff
\svnid{$Id: KNEAD_GlobalFormatting.tex 142 2025-08-03 20:42:20Z KneadProject $}
...
%%
%% This seems to NEED to be first, else it collides with lots of other stuff.
%% These colisions cause TOC,LOT, LOF to be wonky and paragraph spacing to be narrower than expected.
%% As of 05 July 2024, this placement seems to work ;) 
%
%https://tex.stackexchange.com/questions/42520/apparent-conflict-when-using-hyperref-and-pdfcomment-together
%\hypersetup{pdfborder={0 0 0},plainpages=false,pdfpagelabels,breaklinks,hyperfootnotes=true}
%%%
%%%%% This makes the pdf file contain active links for all references,
%%%%% urls, the table of contents, etc., and creates a pdf menu.
%\usepackage[pdfborder={0 0 0},plainpages=false,pdfpagelabels,breaklinks,hyperfootnotes=true,pdfa=true]{hyperref} 
\usepackage[pdfborder={0 0 0},plainpages=false,pdfpagelabels,breaklinks,hyperfootnotes=true]{hyperref} 

\newcommand\KNEADlabel[2][]{\phantomsection\def\@currentlabelname{#1}\label{#2}}
\newcommand{\KNEADref}{\nameref}

%%%
%%% These were called out by pdfcomment, but do not seem to be needed explicitly.
%%%
%%%\usepackage{zref}
%%%\usepackage{xkeyval}
%%%\usepackage{etoolbox}
...
%%%\usepackage{soulpos}
%%%
\usepackage{pdfcomment}% needs to be after \usepackage{hyperref} to prevent a rogue comma above the line with the \pdftooltip
...
\usepackage{eforms}
%https://tex.stackexchange.com/questions/447484/adobe-reader-always-asks-for-saving-when-using-eforms-and-hyperref-packages :: pdfa=true
\OpenAction{/S/JavaScript/JS(this.dirty=false;)}

%https://ctan.mirrors.hoobly.com/macros/latex/contrib/etoolbox/etoolbox.pdf
\newcommand{\KNEADshowSignatureFieldIfEmpty}{}

\newcommand{\KNEADSignatureField}[1]{
\ifdefempty{\KNEADshowSignatureFieldIfEmpty}
%isEmpty
{\bf{#1}\\ \sigField{#1}{\textwidth}{1.5cm}\break\rule{\textwidth}{0.5pt}\\}%true case to SHOW the signature block
%else
{\bf{#1}\\%true case to NOT show the signature block
\begin{center}
\em PDF Signature Field To Be Included in Final Draft
\end{center}
\break\rule{\textwidth}{0.5pt}\\}
}
...
%\usepackage[color,DVIps]{changebar}% not applicable with LaTeX=>PDF
...
%%%
%defernumbers=true %% means numbers are by bibliograph order, not citation order
%defernumbers=false %% means numbers are by citation order
%\usepackage[utf8]{inputenc}
%\usepackage[T1]{fontenc}
%\usepackage[ngerman]{babel}
\usepackage[hyperref=true, defernumbers=false, backend=biber,natbib=true,style=numeric,sorting=none]{biblatex}
...
%letterpaper depricated (WLC20211009)
\usepackage{url}
...
%
%%% WLC 2024.03.14
%%% got errors with \begin{document} so I gave up
%
%\usepackage{epstopdf}% for use by LaTeX->PDF to convert EPS images to PDF for inclusion
										 % need to use LaTeX->PDF to insert PDF files, mainly in STR documents
										% not needed if PDFs are made in GIMP from PNGs
\usepackage{pdfpages}% to include scans in STRs
...
% macro for handling multiple requirements and use cases from a common set of files
\newcommand{\AUTONUMRQTVAR}{AUTO}
\newcommand{\AUTONUMUSECASEVAR}{AUTO}
%arg 1 is section level
%arg 2 is manually provided requirement number
...
\newcommand{\UseCaseNumberAM}[2]{\ifthenelse{\equal{\AUTONUMUSECASEVAR}{AUTO}}{\renewcommand{\UseCaseNumberBase}{\getcurrentref{#1}}}{\renewcommand{\UseCaseNumberBase}{#2}}}
...
\newcommand\DIDINFO{}% make sure it's off by default
\newcommand\DIDINFOON{\renewcommand\DIDINFO[1]{\DIDINFOBASE{##1}}}
%{##1} needed per https://tex.stackexchange.com/questions/367918/how-to-fix-illegal-parameter-number-in-definition-of-a-new-tikz-command
\newcommand\DIDINFOOFF{\renewcommand\DIDINFO[1]{}}
\DIDINFOON% make sure it's on by default
...
\newcommand{\KNEADacronymEntryStart}{\begin{minipage}{\KNEADacronymDefinitionColumnWidth}\vspace{2.5pt}}
\newcommand{\KNEADacronymEntryClose}{\vspace{2.5pt}\end{minipage}\\	\hline}
...
\newcommand{\KNEADglossaryTermArrayStretch}{1.2}
\newlength{\KNEADglossaryTermColumnWidth}
\setlength{\KNEADglossaryTermColumnWidth}{4.8in}% adjust as necessary based on definitions
