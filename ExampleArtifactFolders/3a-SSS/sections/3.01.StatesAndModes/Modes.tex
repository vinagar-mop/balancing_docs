
A summary of the modes is provided in Table~\ref{tab:Modes}.
This table also provides a list of the sub-states in which each mode is valid.
See the formal specifications, if applicable, in the following sections for formal statement of the mode requirements, and accompanying notes that provide further clarification on the meanings of the states.
\begin{table}[htbp]
	\begin{center}
		\begin{tabular}{|p{1.0in}|p{4.0in}|p{1.0in}|}
			\hline
			\hline
			     \multicolumn{3}{|c|}{{\bf MODES}}\\
			\hline
{\bf Name} & {\bf Summary}	& {\bf Valid Sub-States}\\
			\hline
			\hline
The sy & Mode 1 summary & Sub-State A\\ \hline
Mode 2 & Mode 2 summary & Sub-State B\\ \hline
Mode 3 & Mode 3 summary & Sub-State C\\ 
			\hline
			\hline
			\end{tabular}
				\caption{Summary of Modes for \ThisSystem}
				\label{tab:Modes}
		\end{center}
\end{table}

%%%
%%% include subsections and/or files with requirements for the sub-states as needed
%%%

%%% \ONERQMTV[9] is macro for consistently formatted requirements
\MULTIRQMTVKPP
% #1 is requirement Number
{\RqtNumberBase.1}
% #2 is Title
{Mode One}
% #3 is requirement label (expected to be of form rqt:XXX)
{rqt:ModeOne}
% #4 is Text of the specification
{The system shall provide the Mode-1 mode with the following KPP thingies:}
% #5 is Status (S in {(T), (O), (I), (D)} listed by phase as \item [] S)
{
	\item Thingie one.
	\item Thingie two.
	\item Thingie three.
}
% #5 is Status (S in {(T), (O), (I), (D)} listed by phase as \item [] S)
{
	\item [Phase 1] Must past demonstration
}
% #6 is Acceptance
{This requirement shall be verified by demonstration.}
% #7 is Traceability
{
	\item [\cite{ref__KNEAD_Manual}] KPP-1
}
% #8 is Notes; listed as enumeration \item ...
{
	\item The Mode-1 mode generalizes the case where the system has to have the requirement integrated properly within the system.
}
% #9 is changebar version
{P1}
%%%%% end \ONERQMTV[9] macro


%%% \ONERQMTV[9] is macro for consistently formatted requirements
\MULTIRQMTVKSA
% #1 is requirement Number
{\RqtNumberBase.2}
% #2 is Title
{Mode Two}
% #3 is requirement label (expected to be of form rqt:XXX)
{rqt:ModeTwo}
% #4 is Text of the specification
{The system shall provide the Mode-2 mode with these KSA thingamabobs}
% #5 is Status (S in {(T), (O), (I), (D)} listed by phase as \item [] S)
{
	\item Thingamabobs one.
	\item Thingamabobs two.
	\item Thingamabobs three.
}
% #5 is Status (S in {(T), (O), (I), (D)} listed by phase as \item [] S)
{
	\item [Phase 1] Must past demonstration
}
% #6 is Acceptance
{This requirement shall be verified by demonstration.}
% #7 is Traceability
{
	\item [\cite{ref__KNEAD_Manual}] KSA-2
}
% #8 is Notes; listed as enumeration \item ...
{
	\item The Mode-2 mode generalizes the case where the system has to have the requirement integrated properly within the system.
}
% #9 is changebar version
{P1}
%%%%% end \ONERQMTV[9] macro


%%% \ONERQMTV[9] is macro for consistently formatted requirements
\MULTIRQMTV
% #1 is requirement Number
{\RqtNumberBase.3}
% #2 is Title
{Mode Three}
% #3 is requirement label (expected to be of form rqt:XXX)
{rqt:ModeThree}
% #4 is Text of the specification
{The system shall provide the non-KPP and non-KSA Mode-3 mode with these whachamacallits.}
% #5 is Status (S in {(T), (O), (I), (D)} listed by phase as \item [] S)
{
	\item Whachamacallits one.
	\item Whachamacallits two.
	\item Whachamacallits three.
}
% #5 is Status (S in {(T), (O), (I), (D)} listed by phase as \item [] S)
{
	\item [Phase 1] Must past demonstration
}
% #6 is Acceptance
{This requirement shall be verified by demonstration.}
% #7 is Traceability
{
	\item [\cite{ref__KNEAD_Manual}] Multi-3
}
% #8 is Notes; listed as enumeration \item ...
{
	\item The Mode-3 mode generalizes the case where the system has to have the requirement integrated properly within the system.
}
% #9 is changebar version
{P1}
%%%%% end \ONERQMTV[9] macro